import styles from './PasswordSafe.module.scss'
...
import { ModifySafePasswordApi, InfoApi } from '../../request/api'
...
const Password = () => {
	...
	const confirmPassword = () => {
		...
		if (!errors.length || (errors.length == 1 && !hasSafePassword)) {
			let data = {
				newPassword: hex_md5(fields[1].input.state.value),
				oldPassword: hasSafePassword ? hex_md5(fields[0].input.state.value) : null
			}
			ModifySafePasswordApi(data).then(res => {
				if (res.code === 200) {
					setShowToast(true)
					setTimeout(() => {
